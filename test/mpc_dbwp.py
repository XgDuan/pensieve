...
def build_predictor(checkpoint_path):
    checkpoint = torch.load(checkpoint_path)
    args = checkpoint['args']
    Model = importlib.import_module('models.%s_model' % args['model']).Model
    model = Model(**args)
    model.load_state_dict(checkpoint['best_state_dict'])
    global LOG_FILE
    LOG_FILE = './results/log_sim_%s' % (args['model'])

    def predictor(x):
        """
        x: np.ndarray of shape (5,)
        """
        input_tensor = torch.from_numpy(x).unsqueeze(0).to(torch.float32)  # 1* 5
        pred = model.infer_batch(input_tensor, None).squeeze()  # (1,)
...
def main():
    ...
    while True:  # serve video forever
        ...
        # add
        # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        harmonic_bandwidth = predictor(past_bandwidths)
        # /////////////////////////////////////////////////////////////////////
